angular.module('orsApp.landmark-service', []).factory('orsLandmarkService', ['$q', '$http', 'orsUtilsService', 'orsObjectsFactory', 'orsMapFactory', 'lists', 'orsNamespaces', 'ENV', ($q, $http, orsUtilsService, orsObjectsFactory, orsMapFactory, lists, orsNamespaces, ENV) => {
    ...
    orsLandmarkService.prepareQuery = (geom, segments) => {
        ...
        if(segments.length > 0) {
            ...
            for(var j=0; j<segments.length; j++) {
                ...
                var wps = [];
                for(var i=1; i<segment.steps.length; i++) { // dont do the first line segmetn as there is no "instruction" for that
    ...
    orsLandmarkService.showLandmarks = (landmarks) => {
            let action = orsObjectsFactory.createMapAction(10, lists.layers[9], landmarks, undefined, undefined);
    ...
    orsLandmarkService.clearAll = () => {
        let action = orsObjectsFactory.createMapAction(2, lists.layers[9], undefined, undefined, undefined);
        orsMapFactory.mapServiceSubject.onNext(action);
    }
